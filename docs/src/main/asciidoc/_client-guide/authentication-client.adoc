...
=== Elytron client SSL context security provider

You can register `org.wildfly.security.auth.client.ClientSSLContextProvider` security provider. This java security provider loads elytron client configuration and provides initialized ssl context for all clients requesting SSL context by using `SSLContext.getDefault()`.

The provider loads the ssl context from either current authentication context obtained from classpath, or from authentication context obtained from file whose path is passed into the security provider either programmatically or as an argument in `java.security` file.

The SSL context configured to match all rules is the one that will be initialized and returned by this provider:
