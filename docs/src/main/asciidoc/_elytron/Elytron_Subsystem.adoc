...
|distributed-realm |A security realm definition for authentication and authorization identities distributed between multiple security realms.

|failover-realm |A realm definition that is an aggregation of two realms, one for default behaviour and
...
|dynamic-client-ssl-context |An SSL context for use on the client side of a
connection. It delegates to alternate instances based on peer information
and rules provided in its authentication-context attribute.
...
==== Reload WildFly for the changes to take effect.
