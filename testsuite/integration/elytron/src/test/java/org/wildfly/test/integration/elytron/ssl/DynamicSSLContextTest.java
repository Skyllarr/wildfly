...
import org.wildfly.security.auth.client.DynamicSslContextSpi;
import org.wildfly.security.auth.client.DynamicSSLContext;
...
public class DynamicSSLContextTest {
    ...
    private static final String RESOURCES = "src/test/resources/org/wildfly/test/integration/elytron/ssl/";
    private static final String MUTUALSSLSERVERKEYSTORE = "server1.keystore.jks";
    private static final String MUTUALSSLSERVERTRUSTSTORE = "server1.truststore.jks";
    private static final String MUTUALSSLSERVERKEYSTORE2 = "server2.keystore.jks";
    private static final String MUTUALSSLSERVERTRUSTSTORE2 = "server2.truststore.jks";
        ...
        Runnable runnable =
                () -> {
                    try {
                        ...
                        runServerWithMutualSSLThenTestClientAndReturnServerConfigToDefault(SERVER1, DEPLOYMENT_1, MUTUALSSLSERVERKEYSTORE, MUTUALSSLSERVERTRUSTSTORE, client, "archive1.war", 11443, 3000);
                        runServerWithMutualSSLThenTestClientAndReturnServerConfigToDefault(SERVER2, DEPLOYMENT_2, MUTUALSSLSERVERKEYSTORE2, MUTUALSSLSERVERTRUSTSTORE2, client, "archive2.war", 14443, 6000);
                        runServerWithMutualSSLThenTestClientAndReturnServerConfigToDefault(SERVER3, DEPLOYMENT_3, MUTUALSSLSERVERKEYSTORE3, MUTUALSSLSERVERTRUSTSTORE3, client, "archive3.war", 17443, 9000);
                    } catch (Exception e) {
                        e.printStackTrace();
    ...
    private static void secureServerWithMutualSSL(String serverKeystorePath, String serverTruststorePath, int portOffset) throws Exception {
        ...
        ModelNodeResult result = runCmd(client, String.format("/subsystem=elytron/key-store=twoWayKS:add(path=%s,credential-reference={clear-text=%s},type=JKS)", serverKeystorePath, PASSWORD));
        result = runCmd(client, String.format("/subsystem=elytron/key-store=twoWayTS:add(path=%s,credential-reference={clear-text=%s},type=JKS)", serverTruststorePath, PASSWORD));
        result = runCmd(client, "/subsystem=elytron/key-manager=twoWayKM:add(key-store=twoWayKS,credential-reference={clear-text=secret})");
        result = runCmd(client, "/subsystem=elytron/trust-manager=twoWayTM:add(key-store=twoWayTS)");
        result = runCmd(client, "/subsystem=elytron/server-ssl-context=twoWaySSC:add(key-manager=twoWayKM,protocols=[\"TLSv1.2\"],trust-manager=twoWayTM,need-client-auth=true)");
        ...
        client.executeCli("/subsystem=undertow/server=default-server/https-listener=https:write-attribute(name=ssl-context,value=twoWaySSC)\nrun-batch");
    ...
    private static void removeTLSConfigFromJbossHomeServer(int portOffset, String archiveName) throws Exception {
        ...
        client.executeCli("/subsystem=undertow/server=default-server/https-listener=https:undefine-attribute(name=ssl-context)\nrun-batch");
        client.executeCli("run-batch");
        ModelNodeResult result = runCmd(client, "/subsystem=elytron/server-ssl-context=twoWaySSC:remove()");
        result = runCmd(client, "/subsystem=elytron/trust-manager=twoWayTM:remove()");
        result = runCmd(client, "/subsystem=elytron/key-manager=twoWayKM:remove()");
        result = runCmd(client, "/subsystem=elytron/key-store=twoWayTS:remove()");
        result = runCmd(client, "/subsystem=elytron/key-store=twoWayKS:remove()");
        Administration admin = new Administration(client, 240);
        admin.reload();
        client.close();
    }
